<template>
	<view>
		<AppletHeader :title="isUpdate ? '编辑商品' : '新增商品'" 
		right-icon="保存" :right-icon-size="14"
			...
			<view class="box">
				<u--form labelPosition="left" labelWidth="100">
					<u-form-item label="进货价" borderBottom>
						<u--input prefixIcon="￥" v-model="priceForm.purchaseDecimal" placeholder="0.00" border="none"></u--input>
					</u-form-item>
					<u-form-item label="零售价" borderBottom>
						<u--input prefixIcon="￥" v-model="priceForm.commodityDecimal" placeholder="0.00" border="none"></u--input>
					</u-form-item>
					<u-form-item label="批发价" borderBottom>
						<u--input prefixIcon="￥" v-model="priceForm.wholesaleDecimal" placeholder="0.00" border="none"></u--input>
					</u-form-item>
				</u--form>
			</view>
...
import { getMaxBarCode, updateMaterial, addMaterial } from '@/apis'
import { UPLOAD_FILE_URL, IMAGE_OSS_URL } from '@/common/contanst.js'
export default {
	components: {
		SelectShop
	},
	...
	methods: {
		...
		genEditData() {
			...
			let { commodityDecimal, purchaseDecimal, remark, wholesaleDecimal } = cacheData || {}
			...
			this.fileList1 = imgList.map(item => ({ url: item }))
			this.remark = remark
		},
		scanQRcode() {
			console.log('扫码')
		...
		async handleRightClick() {
			...
			let name = this.isUpdate ? this.baseForm.name : `${storeName}${this.baseForm.name}-${this.formatDate()}`
			const params = {
				// 选择的店铺名+时间戳
				name,
				remark: this.remark,
			...
			const { data, code } = await FN(params)
			uni.showToast({
				title: '操作成功',
				icon: 'none'
			})
