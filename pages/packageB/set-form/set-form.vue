<template>
	<view class="main">
					...
					<u-swipe-action>
						<u-swipe-action-item :options="options" v-for="(item, index) of cacheSelectList" :key="item.id"
							...
							<view class="swipe-action flex u-border-bottom">
								<u--image radius="10" :showLoading="true" :src="item.imgName" width="60px"
									height="60px"></u--image>
		...
		<u-datetime-picker ref="datetimePicker" :show="timeShow" v-model="curTimeTemp" mode="date"
			@confirm="confirmTime" @cancel="timeShow = false" :formatter="formatter"></u-datetime-picker>
	</view>
</template>
		...
		methods: {
			onPriceChange: debounce(function(item) {
				...
				if (item.id === 1) {
					...
					this.priceList.find((i) => i.id === 2).value = this.formatMoney(this.totalPrice * (1 -
						discountRate)) // 更新优惠金额
					this.priceList.find((i) => i.id === 3).value = this.formatMoney(this.totalPrice *
						discountRate) // 更新折后金额
				}
				// 优惠金额
				if (item.id === 2) {
					...
					if (this.totalPrice !== 0) {
						...
						this.priceList.find((i) => i.id === 3).value = this.formatMoney(
							newDiscountLastMoney) // 更新折后金额
				...
				if (item.id === 3) {
					...
					if (this.totalPrice !== 0) {
						...
						this.priceList.find((i) => i.id === 2).value = this.formatMoney(this.totalPrice -
							discountLastMoney) // 更新优惠金额
					}
				}
				this.calculateTotalAmount()
			}, 300),
			...
			async saveData() {
				...
				const {
					data
				} = await addDepotHeadAndDetail(this.objItem[this.type].params)
