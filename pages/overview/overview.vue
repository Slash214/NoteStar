<template>
	<view :class="isFix ? 'fixed' : ''">
			...
			<view class="nav">
				...
				<view class="key-data">
					<view class="key-data-item" v-for="item in reportData" :key="item.id">
						...
						<view class="price">
							{{ item.id === 2 ? hiddenContent(item.num) : hiddenContent(formatMoney(item.num)) }}
						</view>

						<u--text :text="genSubtitle(item.value)" size="12" color="#ABC4FA"></u--text>
					</view>
				</view>
			</view>
			...
			<view class="main">
				<view class="white mb20 echart">
					...
					<view class="tip">
						...
						<view class="tip-text">
							...
							<text @click="toggleCurrentDay(false)" :class="!currSeven ? 'active ml10' : 'ml10'">
								近30天
							</text>
						</view>
					</view>
					<view class="chart-container">
						...
						<block v-if="timeType === 1">
							<qiun-data-charts type="area" :canvas2d="true" :opts="areaOptions" :ontouch="true"
								:onmovetip="true" :tooltipShow="true" :chartData="chartData" />
						</block>

						<block v-if="timeType === 3">
							<qiun-data-charts type="line" loadingType="0" :canvas2d="true" :opts="lineOptions"
				...
				<view class="white mb20 commodity">
					<view class="flex flex-items-center">
						...
						<u--text @click="gotoBestSellers" suffixIcon="arrow-right" align="right" text="查看更多" size="13"
							color="#A0A0A0"></u--text>
					</view>
				...
				<view class="white">
					<view class="flex">
						...
						<u--text @click="gotoEmployeePer" suffixIcon="arrow-right" align="right" text="查看更多" size="13"
							color="#A0A0A0"></u--text>
					</view>
		...
		<u-picker @cancel="salesVisible = false" @confirm="onSalesConfirm" :show="salesVisible" keyName="label"
...
<script>
	...
	export default {
		...
		methods: {
			genSubtitle(value) {
				let prefix = this.timeType === 1 ? '昨日' : '上月'
				let formattedValue = this.formatToTwoDecimals(value); // 调用格式化函数
				return `${prefix}${this.hiddenContent(formattedValue)}`
			},
			...
			async getData() {
				...
				if (this.timeType === 3) {
					...
					monthChartData.categories = Array.from({
							length: 31
						},
						(_, index) => {
							const day = index + 1
					...
					// 填充上月数据
					monthChartData.series[0].data = Array.from({
							length: 31
						},
						(_, index) => {
					...
					// 填充本月数据
					monthChartData.series[1].data = Array.from({
							length: 31
						},
						(_, index) => {
