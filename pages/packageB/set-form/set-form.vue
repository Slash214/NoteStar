	...
	methods: {
		onPriceChange: debounce(function (item) {
			...
			if (item.id === 1) {
				...
				this.priceList.find((i) => i.id === 2).value = this.formatMoneyFixed(
					this.totalPrice * (1 - discountRate)
				) // 更新优惠金额
				this.priceList.find((i) => i.id === 3).value = this.formatMoneyFixed(this.totalPrice * discountRate) // 更新折后金额
			}
			// 优惠金额
			if (item.id === 2) {
				...
				if (this.totalPrice !== 0) {
					...
					this.priceList.find((i) => i.id === 3).value = this.formatMoneyFixed(newDiscountLastMoney) // 更新折后金额
