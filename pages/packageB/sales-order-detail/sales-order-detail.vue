<template>
	<view>
		<AppletHeader
			:title="
				type === 1 ? (isH ? '销售单历史记录详情' : '销售单详情') : isH ? '进货单历史记录详情' : '进货单详情'
			"
			:right-icon="isDelete || isH ? ' ' : 'more-dot-fill'"
...
<script>
import { formatDateToChinese, formatImageArray, formatMoney } from '@/utils'
...
export default {
	...
	methods: {
		...
		async getData() {
			try {
				const { data } = await getDetailByNumbe({ number: this.number })
				this.id = data.id
				let {
					number,
					depotName,
