	...
	<view class="container">
		...
		<view class="block">
			...
			<view class="grid grid-4">
				<view :class="item.select ? 'grid-item select' : 'grid-item'" v-for="item of timeArray" :key="item.id"
		...
		<view class="block" v-for="item in dataList" :key="item.id">
			...
			<view class="grid">
				<view v-for="el of item.data" :key="el.id" @click="handleClick(item.id, el)"
					:class="el.select ? 'grid-item select' : 'grid-item'">
					{{ el.name }}
				</view>
			</view>
		</view>

		<u-datetime-picker ref="datetimePicker" @cancel="startShow = false" @confirm="confirmStart($event, 1)"
			:formatter="formatter" :show="startShow" v-model="timpStartTime" mode="date"></u-datetime-picker>
		<u-datetime-picker ref="datetimePicker" @cancel="startShow = false" @confirm="confirmStart($event, 2)"
			:formatter="formatter" :show="endShow" v-model="timpEndTime" mode="date"></u-datetime-picker>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			const [r1, r2] = await Promise.all([getDepotByUserId({
				userId: user.id
			}), getUserByDepotId({
				depotId: 0
			})])
			const array = r2.data.map((item) => ({
				...item,
				name: item.userName,
				select: false
			}))
