...
import { timestampToDate, formatTimestamp, formatMoney } from '@/utils'
	...
	methods: {
		...
		async saveData() {
			...
			let rows = this.cacheSelectList.map((item) => {
				const price = this.type === 1 ? parseFloat(item.commodityDecimal) : parseFloat(item.purchaseDecimal)
		...
		handleClickAction(e) {
			...
			// 重新计算总价格
			let totalPrices = this.cacheSelectList.reduce((accumulator, currentItem) => {
				const unitPrice = new Big(this.type === 1 ? currentItem.commodityDecimal : currentItem.purchaseDecimal)
			...
			this.totalPrice = totalPrices.toFixed(2)
		...
		recalculateDiscounts() {
			// 获取整单折扣
			const totalPrices = new Big(this.totalPrice)
			const discountRate = new Big(this.form.discount)
			// 计算折后金额
			// 计算优惠金额
			const discountMoney = totalPrices.times(Big(1).minus(discountRate.div(100)))
			// 计算折后金额
			const discountLastMoney = totalPrices.minus(discountMoney)
